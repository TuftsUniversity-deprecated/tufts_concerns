...
describe Datastreams::ElectionRecord do
  describe "to_solr" do
    it "should only index the year part of the date" do
      f = File.new(File.expand_path(File.join(fixture_path, "../../../spec/fixtures/election_records/us_potus_1792_RECORD-XML.xml")))
      @potus_1792 = Datastreams::ElectionRecord.from_xml(f.read)
    ...
    describe "with some fixtures" do
      before(:each) do
        f = File.new(File.expand_path(File.join(fixture_path, "../../../spec/fixtures/election_records/us_potus_1792_RECORD-XML.xml")))
        @potus_1792 = Datastreams::ElectionRecord.from_xml(f.read)
        f = File.new(File.expand_path(File.join(fixture_path, "../../../spec/fixtures/election_records/al_staterepresentative_madisoncounty_1820_RECORD-XML.xml")))
        @madisoncounty_1820 = Datastreams::ElectionRecord.from_xml(f.read)
